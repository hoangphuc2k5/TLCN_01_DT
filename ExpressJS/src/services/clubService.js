...
const Club = require('../models/Club'); const ClubRegistration = require('../models/ClubRegistration'); const RetakeRequest = require('../models/RetakeRequest'); const User = require('../models/User'); const Subject = require('../models/Subject'); const AcademicYear = require('../models/AcademicYear');
const { ROLES } = require('../constants/roles'); const { schoolScope, personalStudentIds, objectId } = require('./dataScope'); const { targetSchool, reference } = require('./writeScope');
const managers = [ROLES.SUPER_ADMIN, ROLES.SCHOOL_ADMIN, ROLES.ACADEMIC_AFFAIRS];
const listClubs = async actor => Club.find(await schoolScope(actor)).populate('coordinatorId', 'name code').sort({ name: 1 });
const createClub = async (actor, data) => { if (!managers.includes(actor.role)) throw new ApiError(403, 'Khong co quyen tao CLB'); const schoolId = await targetSchool(actor, data.schoolId); if (!data.name?.trim()) throw new ApiError(400, 'Thieu ten CLB'); return Club.create({ schoolId, name: data.name.trim(), description: String(data.description || '').trim(), capacity: Number(data.capacity || 50), coordinatorId: actor._id, status: 'OPEN' }); };
const register = async (actor, id) => { if (actor.role !== ROLES.STUDENT) throw new ApiError(403, 'Chi hoc sinh duoc dang ky CLB'); const club = await Club.findOne({ _id: objectId(id), ...(await schoolScope(actor)) }); if (!club) throw new ApiError(404, 'Khong tim thay CLB'); if (club.status !== 'OPEN') throw new ApiError(409, 'CLB da dong dang ky'); const count = await ClubRegistration.countDocuments({ clubId: club._id, status: 'REGISTERED' }); if (count >= club.capacity) throw new ApiError(409, 'CLB da du suc chua'); try { return await ClubRegistration.findOneAndUpdate({ clubId: club._id, studentId: actor._id }, { clubId: club._id, schoolId: club.schoolId, studentId: actor._id, status: 'REGISTERED' }, { upsert: true, new: true, runValidators: true }); } catch (e) { if (e.code === 11000) throw new ApiError(409, 'Da dang ky CLB'); throw e; } };
const cancelRegistration = async (actor, id) => { if (actor.role !== ROLES.STUDENT) throw new ApiError(403, 'Chi hoc sinh duoc huy dang ky'); const row = await ClubRegistration.findOne({ _id: objectId(id), studentId: actor._id }); if (!row) throw new ApiError(404, 'Khong tim thay dang ky'); row.status = 'CANCELLED'; return row.save(); };
