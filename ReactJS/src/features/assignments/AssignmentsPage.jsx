...
export default function AssignmentsPage() {
  ...
  const columns = [
    ...
    { title: 'Bài của em', render: (_, r) => r.submission ? (r.submission.score == null ? 'Đã nộp' : `${r.submission.score}/${r.maxScore}`) : student && r.status === 'PUBLISHED' ? <Button size="small" onClick={() => { submitForm.resetFields(); setSubmitTarget(r); }}>Nộp bài</Button> : '-' },
  ...
  return <div>
    ...
    <Modal open={!!submitTarget} title={`Nộp bài: ${submitTarget?.title || ''}`} onCancel={() => setSubmitTarget(null)} onOk={() => submitForm.submit()}>
      <Form form={submitForm} layout="vertical" onFinish={async values => { const result = await submitHomeworkApi(submitTarget._id, values); if (result?.EC === 0) { message.success(result.EM); setSubmitTarget(null); load(); } else message.error(result?.EM); }}><Form.Item name="answerText" label="Nội dung bài làm" rules={[{ required: true, whitespace: true }]}><Input.TextArea rows={10} maxLength={20000} showCount /></Form.Item></Form>
    </Modal>
